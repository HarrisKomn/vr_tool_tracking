import cv2
import cv_bridge
import rclpy
from rclpy.node import Node
from sensor_msgs.msg import Image
from geometry_msgs.msg import PointStamped
import torch
import segmentation_models_pytorch as smp
from albumentations import Crop, Normalize, Resize, Compose
from albumentations.pytorch import ToTensorV2
from vis_utils.visualise_masks import overlay_mask_bin, extract_tool_tip_and_focus_region
import time
import numpy as np

VIS_CV =  True

if not VIS_CV:
    import cupy as cp
    from vispy import app, scene, gloo

    
    # Create a VisPy canvas
    canvas = scene.SceneCanvas(keys="interactive", size=(1920, 1078))
    view = canvas.central_widget.add_view()

    image_data = np.random.rand(1920, 1078, 3)

    # Use ImageVisual correctly
    image_visual = scene.visuals.Image(image_data, parent=view.scene)
    canvas.show()
    #app.process_events()  # Ensure OpenGL context is initialized



class Options:
    def __init__(self):
        
        ####TODO 
        self.model = "Segformer" # Unet, DeepLabV3Plus, UPerNet, Segformer
        self.encoder = "mit_b0"  # resnet34, resnet50, resnet101, mobilenet_v2,mit_b0,mit_b3

        self.resize_img_w = 512
        self.resize_img_h = 512

        self.bbox = (100, 0 , 1180, 1080) #SDI


        self.ckpt_path = "./checkpoints/DGX_SIMPLE_2025-04-03_15-09-02/segm_model.pth"

        self.test_thresh_value = 0.9
        self.batch_size = 1


class ImageSubscriber(Node):
    def __init__(self):
        super().__init__('image_subscriber')
        self.bridge = cv_bridge.CvBridge()
        self.subscription = self.create_subscription(
            Image,
            '/decklink/image_raw',  # Replace with your actual image topic
            self.on_image,
            1
        )

        # Publisher for the processed image
        self.image_publisher = self.create_publisher(
            Image,
            '/output_image_topic', 1  # Replace with your output image topic
        )

        # Publisher for the 3D point coordinates
        self.point_publisher = self.create_publisher(
            PointStamped,
            '/output_point_topic', 1  # Replace with your 3D point topic
        )


        self.frame_counter = 0
        self.window_frames = []

        self.device = 'cuda'

        ### model 
        opt = Options()

        self.ckpt_path = opt.ckpt_path

        if opt.model == 'Unet':
            model = smp.Unet(opt.encoder, encoder_weights=None, classes=1, activation=None)

        elif opt.model == 'DeepLabV3Plus':
            model = smp.DeepLabV3Plus(opt.encoder, encoder_weights=None, classes=1, activation=None)

        elif opt.model == 'Segformer':
            model = smp.Segformer(opt.encoder, encoder_weights=None, classes=1, activation=None)

        model.to(self.device)
        model.eval()
        state = torch.load(self.ckpt_path, map_location=lambda storage, loc: storage)
        model.load_state_dict(state["state_dict"])

    
        self.mean, self.std = (0.485, 0.456, 0.406), (0.229, 0.224, 0.225)

        self.model =  model
        self.resize_img_w, self.resize_img_h = opt.resize_img_w, opt.resize_img_h
        self.test_thresh_value = opt.test_thresh_value

        self.bbox = opt.bbox
        self.test_transform = self.get_transform(self.bbox)
        self.fps_list = []


    def log_fps(self, start_time, end_time):
        
        processing_time = end_time-start_time
        fps = 1/processing_time if processing_time>0 else float('inf')
        self.fps_list.append(fps)

        print(f"Image {self.frame_counter}: FPS {fps:.2f}")


    def get_transform(self, bbox):
        list_trans=[]
        x_min, y_min, w, h = bbox[0], bbox[1], bbox[2], bbox[3],


        list_trans.extend([Crop(x_min=x_min, y_min=y_min, x_max=(x_min+w), y_max=(y_min+h)),
                        Resize(width=self.resize_img_w, height=self.resize_img_h),
                        Normalize(mean=self.mean, std=self.std, p=1),
                        ToTensorV2()])

        list_trans=Compose(list_trans)
        return list_trans


    def prepare_frames(self, input_frame):

        prep_frame = self.test_transform(image = input_frame)['image']
        input_frame = torch.from_numpy(input_frame)

        return prep_frame, input_frame


    def on_image(self, msg):
        # Convert the ROS image message to OpenCV format
        input_frame = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
        prep_frame, input_frame = self.prepare_frames(input_frame)
        self.process_frame(prep_frame, input_frame)


    def process_frame(self, prep_frame, input_frame):

        start_time = time.time()
        with torch.no_grad():
            images = prep_frame.unsqueeze(0)
            images = images.to(self.device)

            
            input_img = input_frame.unsqueeze(0)
            input_img = input_img.to(self.device)

            x, y, w, h = self.bbox
            cropped_img = input_img[:,y:y + h, x:x + w,:]

            pred_mask = self.model(images)
            batch_preds = torch.sigmoid(pred_mask)

            binary_mask = (batch_preds > self.test_thresh_value).long()


            # Overlay masks on image
            overlayed_image = overlay_mask_bin(cropped_img, binary_mask, alpha=0.5)

            # Estimate/overlay tip of tool if tool is present
            if binary_mask.max()>0.:
                overlayed_image = extract_tool_tip_and_focus_region(overlayed_image, binary_mask.squeeze(0), 'left')



            if VIS_CV:
                # overlayed_image_np = overlayed_image.cpu().numpy()[0]
                # cv2.imshow("Segmentation cropped", overlayed_image_np)
                # cv2.waitKey(1)


                #Overlay masks on the whole image
                whole_img = input_img
                whole_img[:,y:y + h, x:x + w, :] = overlayed_image
                whole_img_np = whole_img.cpu().numpy()[0]

                if len(self.fps_list) > 0:
                    fps_text = f"FPS: {self.fps_list[-1]:.2f}"
                    cv2.putText(whole_img_np, fps_text, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255, 0), 2, cv2.LINE_AA)

                cv2.imshow("Segmentation", whole_img_np)
                cv2.waitKey(1)
            
            else:
                # Convert Torch tensor to CuPy (stays in GPU)
                cupy_array = cp.asarray(input_img.squeeze().to(torch.float32)/255.0)

                # Convert CuPy → NumPy only for OpenGL texture (remains in GPU otherwise)
                cupy_image = cp.asnumpy(cupy_array)
                image_visual.set_data(cupy_image)

                # Create an OpenGL texture (RGBA format)
                #texture = gloo.Texture2D(cupy_image, format="rgb", internalformat="rgba32f")

                canvas.update()  # Explicitly request a redraw
                #app.process_events()  # Ensure OpenGL context is initialized
                #canvas.show()



            # Record the total time for this frame (including visualization)
            end_time = time.time()
            self.log_fps(start_time, end_time)


            # # Convert the processed cv2 image back to a ROS image message
            # ros_image = self.bridge.cv2_to_imgmsg(whole_img, encoding='bgr8')

            # # Publish the processed image
            # self.image_publisher.publish(ros_image)


        self.frame_counter += 1

        if self.frame_counter % 100 == 0:
            print(f"Mean (std): FPS {np.mean(self.fps_list):.2f} ({np.std(self.fps_list):.2f})")

def main():

    rclpy.init()
    image_subscriber = ImageSubscriber()

    # Spin the ROS2 node to start receiving messages
    rclpy.spin(image_subscriber)

    # Shutdown the ROS2 client library when done
    rclpy.shutdown()

    app.run()


if __name__ == '__main__':
    main()
